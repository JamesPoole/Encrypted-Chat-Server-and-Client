 ...
 function _upload(file, options) {
  ...
  fileReader.onloadend = function() {
   ...
   function sendChunk() {
    ...
    // If encrypted chat is enabled
    if ($('#encryptChat').is(":checked")) {
     // console.log("write function chunk: ");
     if (fileAESKey) { // only generate the AES key once
      importAES(fileAESKey, fileIv).then((key) => {
       encryptFileAES(key, chunk, fileIv).then((encryptedChunk) => {
        // decryptFileAES(key, encryptedChunk, fileIv).then((newChunk) => { });
        // console.log("encrypted chunk uploading: ");
        // console.log(encryptedChunk); // 10256

        socket.once('socket.io-file::request::' + uploadId, sendChunk);
        socket.emit('socket.io-file::stream::' + uploadId, encryptedChunk);

        fileInfo.sent += encryptedChunk.byteLength;
        self.uploadingFiles[uploadId] = fileInfo;
       });
      });
     } else {
      fileIv = window.crypto.getRandomValues(new Uint8Array(12));
      generateAES().then((key) => {
       exportAES(key).then((exportedAESKey) => {
        fileAESKey = exportedAESKey;
        encryptFileAES(key, chunk, fileIv).then((encryptedChunk) => {
         // decryptFileAES(key, encryptedChunk, fileIv).then((newChunk) => { });
         // console.log("encrypted chunk uploading: ");
         // console.log(encryptedChunk); // 10256
